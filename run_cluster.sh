...
start_cluster() {
    ...
    echo ""
    echo "  # Set a key (to leader)"
    echo "  curl -X POST http://127.0.0.1:9001/kv/mykey \\"
    echo "       -H 'Content-Type: application/json' \\"
    echo "       -d '{\"value\": \"myvalue\"}'"
    echo ""
    echo "  # Get a key (linearizable read)"
    echo "  curl http://127.0.0.1:9001/kv/mykey"
    echo ""
    echo "  # Delete a key"
    echo "  curl -X DELETE http://127.0.0.1:9001/kv/mykey"
    echo ""
    echo "  # Set many keys to see automatic snapshots:"
    echo "  for i in {1..15}; do curl -X POST http://127.0.0.1:9001/kv/key\$i \\"
    echo "       -H 'Content-Type: application/json' \\"
    echo "       -d \"{\\\"value\\\": \\\"value\$i\\\"}\"; done"
    echo ""
    echo "Logs: $LOG_DIR/"
